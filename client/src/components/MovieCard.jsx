...
export default function MovieCard({ movie }) {
  ...
  return (
    <article className="group relative h-48 w-36 flex-none overflow-hidden rounded-sm bg-white/5 shadow-lg transition duration-300 hover:z-20 hover:scale-105 sm:h-56 sm:w-40 md:h-64 md:w-48">
      <img src={movie.thumbnail} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-110" />
      ...
      <div className="absolute inset-x-0 bottom-0 p-3">
        ...
        <div className="mt-3 flex translate-y-4 gap-2 opacity-0 transition group-hover:translate-y-0 group-hover:opacity-100">
          <Link to={`/watch/${movie._id}`} className="grid h-9 w-9 place-items-center rounded-full bg-white text-ink"><Play size={16} fill="currentColor" /></Link>
          <button onClick={() => dispatch(toggleFavorite(movie))} className={`grid h-9 w-9 place-items-center rounded-full ${saved ? 'bg-ember' : 'bg-white/18'}`}><Heart size={16} fill={saved ? 'currentColor' : 'none'} /></button>
        </div>
      </div>
    </article>
  );
